package com.sky.books.order.dao;
...
public class OrderDao {
	...
	public int getStateByOid(String oid) {
		try {
			...
			return (Integer)runner.query(sql, new ScalarHandler(), oid);
		} catch(SQLException e) {
			throw new RuntimeException(e);
		}
	}
	...
	public List<Order> findByState(int state) {
		try {
			...
			List<Order> orderList = runner.query(sql, new BeanListHandler<Order>(Order.class), state);
			for(Order order : orderList) {
				loadOrderItems(order);//为order对象添加它的所有订单条目
			}
			return orderList;
